...
module.exports = (function() {
    ...
    Poloniex.prototype = {
        ...
        _request: function(options, callback){
            ...
            PoloniexLogger.info("OPTIONS: " + JSON.stringify(options));
        ...
        _private: function(parameters, callback){
            ...
            parameters.nonce = nonce() + 14453776208478400;
            options = {
                method: 'POST',
                url: PRIVATE_API_URL,
                form: parameters,
        ...
        getChartData: function(currencyA, currencyB, callback, startEpochSeconds, endEpochSeconds, candlestickPeriod) {
            console.log(startEpochSeconds)
            console.log(endEpochSeconds)
            var parameters = {
                command: 'returnChartData',
                currencyPair: joinCurrencies(currencyA, currencyB),
                start: startEpochSeconds,
                end: endEpochSeconds ? endEpochSeconds : 9999999999,
                period: candlestickPeriod ? candlestickPeriod : 300
            };
            return this._public(parameters, callback);
        },
        ...
        myDepositAddresses: function(callback) {
            var parameters = {
        ...
        myDepositsWithdrawals: function(startEpochSeconds, endEpochSeconds, callback) {
            var parameters = {
        ...
        myTradeHistory: function(currencyA, currencyB, startEpochSeconds, endEpochSeconds, callback){
            var parameters = {
                    command: 'returnTradeHistory',
                    currencyPair: joinCurrencies(currencyA, currencyB),
                    start: startEpochSeconds,
        ...
        buy: function(currencyA, currencyB, rate, amount, callback) {
            if (!rate)   {
                PoloniexLogger.error("Missing rate");
                return new Promise((resolve,reject) => reject("Buy error: Missing rate"));
            }
            if (!amount) {
                PoloniexLogger.error("Missing amount");
                return new Promise((resolve,reject) => reject("Buy error: Missing amount"));
            }
            if (amount < 0) {
                PoloniexLogger.error(`Negative amount ${amount}`);
                return new Promise((resolve,reject) => reject(`Buy error: Negative amount ${amount}`));
            }
            var parameters = {
                    command: 'buy',
                    currencyPair: joinCurrencies(currencyA, currencyB),
